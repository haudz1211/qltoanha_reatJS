...
const MonthlyStatistics = () => {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            console.log('Dispatching getAllMonthlyStatsOfCompanies with month:', months[0]);
            dispatch(getAllMonthlyStatsOfCompanies(months[0]));
        }, 300);
    }, [dispatch, location.pathname, months]);

    useEffect(() => {
        console.log('Statistics updated:', statistics); // Kiểm tra statistics
    }, [statistics]);
    ...
    const onSelectChanged = (e) => {
        ...
        setTimeout(() => {
            ...
            console.log('Selected month:', monthInt, 'Year:', yearInt);

            let selectedMonthItem = null;
            months.forEach(month => {
                if (month.month === parseInt(monthInt) && month.year === parseInt(yearInt)) {
                    selectedMonthItem = month;
                }
            });
            console.log('Selected month item:', selectedMonthItem);
